...
This script tests deployed SageMaker endpoints with sample fashion items.
It downloads images from S3, sends them to the endpoint, and displays recommendations.
...
Usage:
    ...
    python scripts/testing/test_endpoint.py --save-visual  # Creates visual output with images + JSON
...
def get_endpoint_name():
    ...
    # Try to read from endpoint_info.json in sagemaker directory
    for path in ['sagemaker/endpoint_info.json', '../sagemaker/endpoint_info.json', '../../sagemaker/endpoint_info.json']:
        info_file = Path(path)
        if info_file.exists():
...
def create_visual_output(input_image_path, recommendations, result, output_dir='scripts/testing/test_results'):
